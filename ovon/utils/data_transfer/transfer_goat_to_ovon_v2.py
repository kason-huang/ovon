...
#only fetch one type task

def process_one_file_for_task_type(in_path: str, out_path: str, task_type:str) -> int:
    """处理一个 content/ 下的 json.gz 文件"""
    with gzip.open(in_path, "rt", encoding="utf-8") as f:
        data = json.load(f)
    ...
    for ep in data.get("episodes", []):
        ...
        for task in tasks:
            # 跳过 description 模态
            if len(task) > 1 and task[1] != task_type:
                continue
...
def process_tmp():
    ...
    task_type = "object"
    in_split_dir = os.path.join(input_root, split)
    out_split_dir = os.path.join(output_root, f"{split}_{task_type}")
    os.makedirs(out_split_dir, exist_ok=True)
    ...
    max_workers = min(32, len(tasks_list))
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = {ex.submit(process_one_file_for_task_type, in_path, out_path, task_type): (in_path, out_path) for in_path, out_path in tasks_list}
