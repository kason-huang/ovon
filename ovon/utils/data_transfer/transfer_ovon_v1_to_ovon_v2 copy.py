...
input_root = "/root/workspace/lab/ovon/data/datasets/ovon/hm3d/v1/"
...
# 修改点
# 1. goals_by_category里面加一个image goal为[]的字段
# 2. episodes里面加一个tasks: []，内容是["{object_category}, "object", null]
def process_one_file(in_path: str, out_path: str) -> int:
    ...
    for ep in data.get("episodes", []):
        ...
        if object_category == "":
            continue
        task = [object_category, task_type, None]
...
def process_all_splits():
    ...
    for split in splits:
        ...
        out_split_dir = os.path.join(output_root, f"ovon_{split}")
