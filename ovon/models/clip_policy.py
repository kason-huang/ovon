...
from ovon.task.goat_sensors import GoatGoalSensor, GoatModalTypeSensor, GoatRawGoalSensor, GoatGoalRawImageSensor, GoatGoalObjectSensor, GoatGoalMaskSensor
...
class OVONNet(Net):
    def __init__(
        ...
        **kwargs,
        ...
        if backbone in ["clip_attnpool", "siglip", "goatsiglip", "siglip_cl"]:
        ...
        assert ClipObjectGoalSensor.cls_uuid in observation_space.spaces or GoatGoalSensor.cls_uuid in observation_space.spaces or GoatRawGoalSensor.cls_uuid in observation_space.spaces or GoatGoalMaskSensor.cls_uuid in observation_space.spaces
    ...
    # using goat raw with compression layer
    def forward(
        self,
        ...
        # 如果这里报错 "Sizes of tensors must match except in dimension 1. Expected size 2 but got size 16 for tensor number 2 in the list."hi因为prev_action的原因
        # prevaction的size是根据配置文件（当前的）的num_env配置的，但是呢实际运行的num_env是根据ckpt里面的保存的配置导致的
    ...
    # for goat rl（use embedding cache）
    def forward_goat_rl(
        self,
        observations: Dict[str, torch.Tensor],
        rnn_hidden_states,
